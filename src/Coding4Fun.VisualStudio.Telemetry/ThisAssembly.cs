internal sealed class ThisAssembly
{
	internal const string AssemblyVersion = "16.0.0.0";

	internal const string AssemblyFileVersion = "16.3.2.14029";

	internal const string AssemblyInformationalVersion = "16.3.2+gcd36577e57";

	internal const string AssemblyName = "Coding4Fun.VisualStudio.Telemetry";

	internal const string AssemblyConfiguration = "Release";

	internal const string PublicKey = "";

	internal const string PublicKeyToken = "b03f5f7f11d50a3a";

	internal const string RootNamespace = "Coding4Fun.VisualStudio.Telemetry";

	private ThisAssembly()
	{
	}
}
